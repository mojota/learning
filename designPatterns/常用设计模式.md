...
2. Android ListView的Adapter。ListView需要能够显示各式各样的视图，每个人需要的显示效果各不相同，显示的数据类型、数量等也千变万化。

目标接口target:统一输入为View。ItemView千变万化，但终究它都是View类型。

源Adaptee:需要被处理的千变万化的ItemView。

适配器Adapter:通过getView()方法统一将ItemView输出为View。
```
