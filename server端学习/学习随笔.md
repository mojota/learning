...
10. Filter,主要用于对用户请求进行预处理，也可对HttpServletResponse进行后处理。用处：
   1. 在到达Servlet前，拦截客户的HttpServletRequest。
   2. 检查HttpServletRequest,也可修改request头和处理。
   3. 在HttpServletResponse到达客户端之前，拦截HttpServletResponse.
   4. 检查修改HttpServletResponse头和数据。
...
11. 配置Filter
  ...
  ```
12. Filter与Servlet区别：
   1. 极其相似，Filter不能直接响应用户。
   2. doFilter()是从service()里抽取的通用代码，实现复用。
   3. 生命周期相同。
...
24. Struts2提供了一个ActionContext类，action可通过该类来访问Servlet API（HttpServletRequest、HttpSession、ServletContext）。
   ...
   setSession()
   ```
   acton中，通过ActionContext.getContext()来使用。
...
32. Hibernate采用xml映射文件
   ...
   * session保存对象，处于session管理下才有持久化能力（sessionFactory生成的，线程安全）。
   * 事务transaction
